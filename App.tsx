...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'learning' | 'leaderboard' | 'password-tool' | 'url-scanner'>('dashboard');
  ...
  if (view === 'login') {
    return <Login onLoginSuccess={(user, token) => handleLoginSuccess(user, token)} onSwitchToRegister={() => setView('register')} />;
  }
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="fixed bottom-0 left-0 right-0 md:relative md:bottom-auto md:left-auto md:right-auto z-50 md:z-auto bg-black/80 md:bg-white/5 backdrop-blur-xl md:backdrop-blur-none p-2 md:p-1 border-t md:border border-white/10 md:border-white/5 flex items-center justify-around md:justify-start md:gap-1 md:rounded-xl">
            ...
            >
              ...
              <span className="md:inline">Rankings</span>
            </button>
            <button
              onClick={() => setActiveTab('password-tool')}
              className={`flex-1 md:flex-none px-4 py-3 md:py-2 rounded-lg text-xs md:text-sm font-bold transition-all flex flex-col md:flex-row items-center gap-1 md:gap-2 ${activeTab === 'password-tool' ? 'bg-emerald-500 text-black' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}
            >
              <Lock className="w-5 h-5 md:w-4 md:h-4" />
              <span className="md:inline">Auditor</span>
            </button>
            <button
              onClick={() => setActiveTab('url-scanner')}
              className={`flex-1 md:flex-none px-4 py-3 md:py-2 rounded-lg text-xs md:text-sm font-bold transition-all flex flex-col md:flex-row items-center gap-1 md:gap-2 ${activeTab === 'url-scanner' ? 'bg-emerald-500 text-black' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}
            >
              <Globe className="w-5 h-5 md:w-4 md:h-4" />
              <span className="md:inline">Phish Guard</span>
            </button>
          </nav>
          ...
          {gameMode ? (
            ...
            >
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold mb-2">Password Auditor</h2>
                <p className="text-zinc-400 text-sm">Create unbreakable shields for your digital life.</p>
              </div>
              <PasswordTool />
            </motion.div>
          )}
